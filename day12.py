...
dataset[1][start] = 0
# # pt 2
# start = tuple(np.argwhere(dataset[0]==27)[0])
# dataset[0][start] = 26
# dataset[1][start] = 0
...
while True:
    ...
    if cur_node == end:
        # # pt 2
        # if dataset[0][cur_node] == 1:
        break
    for direction in moveset:
        visiting = cur_node[0] + direction[0], cur_node[1] + direction[1]
        if (
            not any(np.array(visiting) < 0)
            and not visiting[0] >= np.shape(dataset[0])[0]
            and not visiting[1] >= np.shape(dataset[0])[1]
        ):
            if (
                dataset[2][visiting] == 0
                and dataset[0][visiting] >= dataset[0][cur_node] - 1
            ):
